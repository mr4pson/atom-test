...
function SmallScreenNavigation(props: Props): JSX.Element {
  ...
  additionalLinks.push({
    name: userInfo?.role === userType.USER ? 'Личный кабинет'
      : !currentJwtPair ? 'Зарегистрироваться'
        : 'Панель администратора',
    path: userInfo?.role === userType.USER ? paths[Page.PRIVATE_OFFICE]
      : !currentJwtPair ? paths[Page.SIGN_UP]
        : '/admin',
  });
  ...
  return <div className={styles['small-screen-navigation']}>
    ...
    <ul className={styles['small-screen-navigation__links-wrapper']}>
      {links.map((link: TypeLink) => (
        <li className={styles['small-screen-navigation__item']} key={link.path}>
          {
